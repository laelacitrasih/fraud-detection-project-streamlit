...
page\_title="📱 Fraud Message Detection",
layout="centered",
...
MODEL\_PATH = "model/model.pkl"
VECTORIZER\_PATH = "model/vectorizer.pkl"

@st.cache\_resource
def load\_model():
try:
if os.path.exists(MODEL\_PATH) and os.path.exists(VECTORIZER\_PATH):
with open(MODEL\_PATH, "rb") as f:
model = pickle.load(f)
with open(VECTORIZER\_PATH, "rb") as f:
...
data\_path = "data/dataset\_sms\_spam\_v1.csv"
